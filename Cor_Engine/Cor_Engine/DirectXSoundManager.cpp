...
DirectXSoundManager::DirectXSoundManager() {
}
...
bool DirectXSoundManager::Initialize(HWND hWnd) {
...
void DirectXSoundManager::Shutdown() {
	ShutdownDirectSound();
}

bool DirectXSoundManager::InitializeDirectSound(HWND hWnd) {
...
void DirectXSoundManager::ShutdownDirectSound() {
	if (primaryBuffer) {
...
bool DirectXSoundManager::LoadWaveFile(const char* path, const char* soundName) {
...
void DirectXSoundManager::ShutdownWaveFile(const char* soundName) {
	if (soundMap[soundName]) {
		soundMap[soundName]->Stop();
	}
}

bool DirectXSoundManager::PlayWaveFile(const char* soundName) {
	...
	if (FAILED(soundMap[soundName]->Play(0, 0, DSBPLAY_LOOPING))) {
		return false;
	}
	return true;
}